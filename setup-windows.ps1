...
"@
Set-Content -Path "$Dir\.claude\settings.local.json" -Value $settings -Encoding UTF8
$md = @'
...
'@
Set-Content -Path "$Dir\CLAUDE.md" -Value $md -Encoding UTF8

# 4. Test
Set-Location $Dir
Write-Host "Testing connection..."
$out = claude -p "Reply with exactly: SETUP OK" 2>$null
